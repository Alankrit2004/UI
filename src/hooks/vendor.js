...
import { CommonConstant, VendorService, ErrorConstant, PaginationType } from "utility";

export const GetVendorList = (paginationType = PaginationType.default) => {
...
export const GetVendorDetail = (Id) => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const result = await VendorService.byId({ Id });
            setData(result.data);
        } catch {
            setError(ErrorConstant.default);
        } finally {
