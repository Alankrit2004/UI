...
import { GetFloorList, GetBuildingList, GetVerifiedList, GetLocationList, GetFireAlarmChecklistList, GetUserList, GetFireAlarmChecklistDetail, reactToaster } from 'hooks';
...
export function AddEditFireAlarmChecklist() {
  ...
  useEffect(() => {
    if (Number(user?.roleId) !== 1) {
      setPermission(getSinglePermission(user?.userRoleList, "FIRE ALARM CHECKLIST"))
    }
  }, [user])
...
export const AddEditFireAlarmChecklistModal = ({ data, mode, open, onClose, onSubmit }) => {
  ...
  const { options: verifiedByList } = GetVerifiedList(PaginationType.all, true)
